...
const AddFundsModal = ({ isOpen, onClose, currentBalance, onFundsAdded }) => {
  ...
  const handleAddFunds = async () => {
    ...
    try {
      ...
      onFundsAdded(); // refresh parent data
      onClose(); // close modal
    } catch (err: any) {
...
const AcquisitionDashboardPage = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      // normalize API data to avoid undefined access
      const apiData = overviewData?.data ?? {};
      setData({
        kpis: apiData.kpis ?? {},
        trend: Array.isArray(apiData.trend) ? apiData.trend : [],
      });

      setWalletBalance(balanceData?.data?.balance ?? null);
    } catch (err: any) {
      setError(
  ...
  // safe defaults to avoid "cannot read properties of undefined"
  const safeData = {
    kpis: data?.kpis ?? {},
  ...
  const metricCards = [
    {
      title: "Spend",
      value: `$${safeData?.kpis?.revenue?.toLocaleString?.() || "0"}`,
      Icon: DollarSign,
    },
    {
      title: "Avg. Conversion",
      value: `${safeData?.kpis?.ctr ?? "0.00"}%`,
      Icon: BarChart2,
    },
    {
      title: "Avg. Daily Users",
      value: safeData?.kpis?.conversions?.toLocaleString?.() || "0",
      Icon: Users,
    },
    {
      title: "Total Clicks",
      value: safeData?.kpis?.clicks?.toLocaleString?.() || "0",
    ...
    <div className="bg-gray-50 min-h-screen p-4 sm:p-6 lg:p-8 font-sans">
      <div className="max-w-7xl mx-auto">
        ...
        <AddFundsModal
          ...
          onFundsAdded={fetchDashboardData} // refresh after adding funds
        />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          {/* Other Filters */}
          {["All Applications", "All Campaigns", "All Countries"].map(
            (filter, i) => (
              <div
                key={i}
                className="w-full bg-white border border-gray-300 px-4 py-2 rounded-lg shadow-sm"
              >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {metricCards.map(({ title, value, Icon }) => (
            <div
              key={title}
              className="bg-white p-5 rounded-xl shadow-md transition-transform transform hover:-translate-y-1"
            >
            ...
            <table className="w-full text-sm text-left text-gray-500">
              ...
              <tbody>
                {loading ? (
                  ...
                  safeData.trend.map((row: any, index: number) => (
                    <tr
                      key={index}
                      className="bg-white border-b hover:bg-gray-50"
                    >
                      <td className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                        {format(new Date(row.day), "MMMM d, yyyy")}
                      </td>
                      ...
                      <td className="px-6 py-4 text-gray-700">
                        {row.conversions?.toLocaleString?.() ?? "0"}
                      </td>
                    </tr>
                  ))
                )}
